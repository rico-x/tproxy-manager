...
local function render(ctx)
  local m, http, sys, fs, disp, jsonc = ctx.m, ctx.http, ctx.sys, ctx.fs, ctx.disp, ctx.jsonc
  local pcdata, fval, fval_last = ctx.pcdata, ctx.fval, ctx.fval_last
  local self_url, redirect_here = ctx.self_url, ctx.redirect_here
  local write_file, read_file, strip_json_comments, write_json_file = ctx.write_file, ctx.read_file, ctx.strip_json_comments, ctx.write_json_file
  ...
  do
    ...
    if http.formvalue("_geo_update_all") == "1" then
      ...
      for _, r in ipairs(rows) do
        if r.url and r.url ~= "" and r.dest and r.dest ~= "" then total = total + 1; if fetch_to(r.url, r.dest) then ok_count = ok_count + 1 end end
      end
    ...
    if http.formvalue("_geo_edit") then
      ...
      if idx then
        http.redirect(self_url({tab="updates",list_file=fval_last("list_file"),json_file=fval_last("json_file"),clash_file=fval_last("clash_file")}) .. "&_geo_edit_idx=" .. idx)
        return m
      end
    end
